# Parseocr Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/parseocr.svg?label=pypi%20(stable))](https://pypi.org/project/parseocr/)

The Parseocr Python library provides convenient access to the Parseocr REST API from any Python 3.9+
...
pip install git+ssh://git@github.com/parseocr/parseocr-python.git
```
...
to add `PARSEOCR_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncParseocr` instead of `Parseocr` and use `await` with each API call:

```python
import os
import asyncio
from parseocr import AsyncParseocr
...
```sh
# install from the production repo
pip install 'parseocr[aiohttp] @ git+ssh://git@github.com/parseocr/parseocr-python.git'
```
...
import os
import asyncio
from parseocr import DefaultAioHttpClient
from parseocr import AsyncParseocr
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `parseocr.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `parseocr.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `parseocr.APIError`.
...
except parseocr.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except parseocr.APIStatusError as e:
...
You can enable logging by setting the environment variable `PARSEOCR_LOG` to `info`.

```shell
$ export PARSEOCR_LOG=info
```
...
```python
import httpx
from parseocr import Parseocr, DefaultHttpxClient

client = Parseocr(
    # Or use the `PARSEOCR_BASE_URL` env var
